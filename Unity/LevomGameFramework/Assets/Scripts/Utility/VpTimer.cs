...
public class VpTimer : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        while ((VpTimer.m_Active.Count > 0) && m_EventBatch < MaxEventsPerFrame)
        {
            ...
            bool isExecuteTime = VpTimer.m_Active[m_EventIterator].IsExecuteTime();
            /*  if (VpTimer.m_Active[m_EventIterator].IsUnscaledTime && Time.unscaledTime >= VpTimer.m_Active[m_EventIterator].DueTime)
                  isExecuteTime = true;
              else if (!VpTimer.m_Active[m_EventIterator].IsUnscaledTime && Time.time >= VpTimer.m_Active[m_EventIterator].DueTime)
                  isExecuteTime = true;
            */

            // execute all due events
            if (isExecuteTime ||   // time is up
            ...
            else
            {
                ...
                {
                    if (VpTimer.m_Active[m_EventIterator].IsUnscaledTime)
                        VpTimer.m_Active[m_EventIterator].DueTime += Time.unscaledDeltaTime;
                    else
                ...
                { // log lifetime
                    if (VpTimer.m_Active[m_EventIterator].IsUnscaledTime)
                        VpTimer.m_Active[m_EventIterator].LifeTime += Time.unscaledDeltaTime;
                    else
        ...
        for (int i = 0; i < m_ActiveDotDestroryOnChangeMap.Count; i++)
        {
            ...
            if (time.IsUnscaledTime && Time.unscaledTime >= time.DueTime || // time is up
                !time.IsUnscaledTime && Time.time >= time.DueTime || // time is up
    ...
    public static int InNotDestroy(float delay, Callback callback)
    {
        ...
        if (m_NewEvent.IsUnscaledTime) m_NewEvent.DueTime = Time.unscaledTime + delay;
        else m_NewEvent.DueTime = Time.time + delay;
    ...
    /// 忽略Time.unScaleTime
    /// </summary>
    public static void InUnscaledTime(float delay, Callback callback, Handle timerHandle = null)
    {
        Schedule(delay, callback, null, null, timerHandle, 1, -1.0f, true);
    }
    ...
    /// </summary>
    private static void Schedule(float time, Callback func, ArgCallback argFunc, object args, Handle timerHandle, int iterations, float interval, bool isUnscaledTime = false)
    {
        ...
        m_NewEvent.IsUnscaledTime = isUnscaledTime;

        if (isUnscaledTime) m_NewEvent.StartTime = Time.unscaledTime;
        else m_NewEvent.StartTime = Time.time;

        if (isUnscaledTime) m_NewEvent.DueTime = Time.unscaledTime + time;
        else m_NewEvent.DueTime = Time.time + time;
    ...
    public static void Pauses(bool b)
    {
        if (m_Active != null)
        {
    ...
    private class Event
    {
        ...
        public bool IsUnscaledTime = false;//是否忽略游戏时间速度
#if (DEBUG && UNITY_EDITOR)
        ...
        public void Execute()
        {
            ...
            if (IsUnscaledTime) DueTime = Time.unscaledTime + Interval;
            else DueTime = Time.time + Interval;
        }
        /// <summary>
        ...
        //是否到了执行时间
        public bool IsExecuteTime(bool isShowLog = false)
        {
            bool isExecuteTime = IsUnscaledTime && Time.unscaledTime >= DueTime ||
               !IsUnscaledTime && Time.time >= DueTime;

            if (isShowLog) Debug.LogFormat("isExecuteTime:{0} , Time.time:{1} ,DueTime:{2}", isExecuteTime, Time.time, DueTime);

            return isExecuteTime;
        }
    }
    ...
    public class Handle
    {
        ...
        public float TimeOfLastIteration
        {
            get
            {
                if (Active)
                {
                    if (m_Event.IsUnscaledTime) return Time.unscaledTime + DurationLeft;
                    else return Time.time + DurationLeft;
                }
                return 0.0f;
            }
        }
        ...
        public float TimeUntilNextIteration
        {
            get
            {
                if (Active)
                {
                    if (m_Event.IsUnscaledTime) return m_Event.DueTime - Time.unscaledTime;
                    else return m_Event.DueTime - Time.time;
                }
                return 0.0f;
            }
        }
        ...
        public void Execute()
        {
            if (m_Event.IsUnscaledTime) m_Event.DueTime = Time.unscaledTime;
            else m_Event.DueTime = Time.time;
        }

        public Callback GetFunction()
        {
            if (m_Event == null) return null;
            return m_Event.GetFunction();
        }

        public bool IsExecuteTime(bool isShowLog = false)
        {
            if (m_Event == null) return false;
            return m_Event.IsExecuteTime(isShowLog);
        }
    }
}
